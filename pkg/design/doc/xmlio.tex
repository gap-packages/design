...
The combinatorial and group-theoretical properties output for each design
depend on <include>, which should be a list containing zero or more of
the strings `"indicators"', `"resolvable"', `"combinatorial_properties"',
`"automorphism_group"', and `"resolutions"'.  A shorthand for the list
containing all these strings is `"all"'. The default for <include> is
the empty list (which is a change from versions of {\DESIGN} previous
to 1.1).  The strings `"indicators"', `"combinatorial_properties"',
